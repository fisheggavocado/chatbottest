...
| 5. 검색 | BM25(kiwipiepy) + 벡터 검색을 RRF(Reciprocal Rank Fusion)로 융합 (`similarity_top_k=5`, `num_queries=1`) |
| 6. 저장·재개 | 페이지 단위 스트리밍 저장 + `checkpoint.json` 기반 중단/재개, 완료 시 HF Dataset에 자동 백업 |
| 7. 챗봇 응답 생성 | (예정) 검색 결과를 프롬프트에 넣어 LLM API 호출 |
...
```bash
python verify_embeddings.py "질문 내용" --top_k 5                # 기본값: 하이브리드(BM25+벡터 RRF)
...
- `verify_embeddings.py` — 저장된 인덱스 검색 검증 스크립트
