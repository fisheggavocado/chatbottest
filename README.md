...
- `.github/workflows/docker-publish.yml` — main에 push할 때마다 이미지를 빌드해 `ghcr.io/fisheggavocado/chatbottest:latest`로 자동 배포 (패키지는 public).
...
| 컨테이너 이미지 | `fisheggavocado/chatbottest:latest` |
| 컨테이너 포트 | `8000` |
...
- gcube 이미지 검증 통과. 다음 단계: HF Dataset PDF 1개 격리 테스트(`test_hf_one_pdf.py`) → 본 실행(`main.py --use-hf`).
